use llm_connector::{LlmClient, types::{ChatRequest, Message, Role, Tool, Function, ToolCall, FunctionCall}};
use serde_json::json;
    ...
    if let Some(choice) = response.choices.first() {
        if let Some(tool_calls) = &choice.message.tool_calls {
            ...
            for (i, msg) in messages.iter().enumerate() {
                println!("    [{}] role={:?}, content={}, tool_calls={}, tool_call_id={:?}", 
                    i, 
                    msg.role, 
            ...
            println!("\n📥 第二轮：LLM 最终响应");
            println!("  finish_reason: {:?}", response2.choices.first().and_then(|c| c.finish_reason.as_ref()));
