...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = ChatRequest::new(&model)
        .add_message(Message::user("Solve this math problem step by step: What is the derivative of x^2 * sin(x)?"))
    ...
    if let Some(reasoning) = response.choices.first().and_then(|c| c.message.reasoning_any()) {
        ...
        println!("⚠️ No explicit reasoning content returned (maybe the model integrates it into the response or doesn't support the 'thinking' parameter yet).");
    }
