//! Test Kimi (Moonshot) Configuration Using Config.kimi Field
//!
//! This test verifies that the kimi field in Config works correctly
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🧪 Testing Kimi (Moonshot) Configuration via Config.kimi");
    println!("======================================================\n");
    ...
    // Create config with kimi field set (without using Default::default())
    println!("\n🔧 Creating Config with kimi field set...");
    let config = Config {
        openai: None,
        anthropic: None,
        deepseek: None,
        zhipu: None,
        aliyun: None,
        kimi: Some(ProviderConfig::new(moonshot_key.clone())),
    };

    println!("✅ Config created with kimi field");
    ...
    println!("\n📝 Testing chat connection using model name 'kimi/moonshot-v1-8k'...");
    let request = ChatRequest {
        model: "kimi/moonshot-v1-8k".to_string(),
        messages: vec![Message::user("Hello! Please respond with 'Kimi configuration successful!'")],
        max_tokens: Some(50),
    ...
    match client.chat(request).await {
        Ok(response) => {
            if let Some(choice) = response.choices.first() {
                ...
                println!("\n🎉 Kimi configuration test completed successfully!");
            } else {
