...
use crate::core::{GenericProvider, HttpClient, Provider, Protocol};
...
pub fn aliyun_international(api_key: &str, region: &str) -> Result<AliyunProvider, LlmConnectorError> {
    aliyun(api_key, &format!("https://dashcope.{}.aliyuncs.com/api/v1", region))
}
...
pub fn anthropic_with_config(
    ...
    Ok(GenericProvider::new(AnthropicProtocol::new(api_key), client))
}
...
impl Provider for TencentProvider {
    ...
    async fn chat(&self, request: &ChatRequest) -> Result<ChatResponse, LlmConnectorError> {
        ...
        let body = serde_json::to_string(&protocol_request).map_err(|e| {
            LlmConnectorError::InvalidRequest(format!("Serialize error: {}", e))
        })?;
        ...
        let response = self
            ...
            .post(
                &self.protocol.chat_endpoint(self.client.base_url(), &request.model),
                &protocol_request,
            )
            .await?;
