...
pub fn deepseek_with_config(
    ...
    let protocol =
        ConfigurableProtocol::openai_compatible(OpenAIProtocol::new(api_key), "deepseek");

    // Use Builder Build
    let mut builder =
        ProviderBuilder::new(protocol, base_url.unwrap_or("https://api.deepseek.com"));
...
mod tests {
    ...
    fn test_deepseek_with_config() {
        let provider =
            deepseek_with_config("sk-test-key", Some("https://custom.url"), Some(60), None);
        assert!(provider.is_ok());
    }
    ...
    fn test_deepseek_protocol_endpoint() {
        let protocol =
            ConfigurableProtocol::openai_compatible(OpenAIProtocol::new("sk-test-key"), "deepseek");
