...
pub fn xiaomi_with_config(
    ...
    let protocol = ConfigurableProtocol::openai_compatible(OpenAIProtocol::new(api_key), "xiaomi");
...
mod tests {
    ...
    fn test_xiaomi_with_config() {
        let provider =
            xiaomi_with_config("test-api-key", Some("https://custom.url"), Some(60), None);
        assert!(provider.is_ok());
    }
    ...
    fn test_xiaomi_protocol_endpoint() {
        let protocol =
            ConfigurableProtocol::openai_compatible(OpenAIProtocol::new("test-api-key"), "xiaomi");
