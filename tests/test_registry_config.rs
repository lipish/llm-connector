...
use llm_connector::config::{ProviderConfig, RegistryConfig};
...
fn test_registry_from_config() {
    ...
    // Add some providers to config
    config.providers.insert(
        "deepseek".to_string(),
        llm_connector::config::ProviderConfigEntry {
            protocol: "openai".to_string(),
            config: ProviderConfig::new("test-key"),
        },
    );
...
fn test_multiple_provider_registration() {
    ...
    let config1 = ProviderConfig::new("deepseek-key")
        .with_base_url("https://api.deepseek.com")
        .with_timeout_ms(30000);

    let config2 = ProviderConfig::new("aliyun-key")
        .with_base_url("https://dashscope.aliyuncs.com")
        .with_timeout_ms(45000);

    let config3 = ProviderConfig::new("zhipu-key")
        .with_base_url("https://open.bigmodel.cn")
        .with_timeout_ms(60000);

    // Register multiple providers
    registry.register("deepseek", config1, deepseek()).unwrap();
    registry
        .register("aliyun", config2, AliyunProtocol::new(None))
        .unwrap();
    ...
    assert_eq!(
        registry.get_provider("deepseek").unwrap().name(),
        "deepseek"
    );
