...
fn test_client_creation() {
    ...
    assert!(LlmClient::zhipu_openai_compatible("test-key", "https://open.bigmodel.cn/api/paas/v4/").is_ok());
...
fn test_client_provider_name() {
    ...
    let aliyun_client = LlmClient::aliyun("sk-test", "https://dashscope.aliyuncs.com/api/v1").unwrap();
    assert_eq!(aliyun_client.provider_name(), "aliyun");

    let anthropic_client = LlmClient::anthropic("sk-ant-test", "https://api.anthropic.com").unwrap();
    assert_eq!(anthropic_client.provider_name(), "anthropic");

    let zhipu_client = LlmClient::zhipu("test-key", "https://open.bigmodel.cn/api/paas/v4/").unwrap();
...
fn test_http_client_creation() {
    ...
    let _client = HttpClient::new("https://api.openai.com")
        .unwrap()
