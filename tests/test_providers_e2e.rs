...
async fn test_aliyun_provider_chat_completion() {
    ...
    test_provider_chat_completion("aliyun", "qwen-turbo", "/api/v1/services/aigc/text-generation/generation").await;
...
async fn test_all_providers_streaming() {
    ...
    test_provider_streaming("aliyun", "qwen-turbo", "/api/v1/services/aigc/text-generation/generation").await;
...
async fn test_provider_registry() {
    use llm_connector::providers::{ProviderRegistry, AliyunAdapter, openai::{deepseek, zhipu}};
    ...
    registry.register("deepseek", config.clone(), DeepSeekAdapter).unwrap();
    registry.register("aliyun", config.clone(), AliyunAdapter).unwrap();
    registry.register("zhipu", config.clone(), ZhipuAdapter).unwrap();
...
async fn test_generic_provider_with_adapters() {
    use llm_connector::providers::{GenericProvider, AliyunAdapter, Provider, openai::{deepseek, zhipu}};
...
async fn test_provider_chat_completion(provider_name: &str, model: &str, endpoint: &str) {
    println!("  📝 Testing {} provider with model {}", provider_name, model);
    ...
    assert_eq!(response.choices[0].message.content, format!("Hello from {} provider!", provider_name));
    assert!(response.usage.is_some());
    
    println!("    ✅ {} provider chat completion test passed", provider_name);
}
...
async fn test_provider_streaming(provider_name: &str, model: &str, endpoint: &str) {
    ...
    println!("  📝 Testing {} provider streaming with model {}", provider_name, model);
    ...
    assert!(!responses.is_empty(), "No streaming responses received for {}", provider_name);
    
    println!("    ✅ {} provider streaming test passed ({} chunks)", provider_name, responses.len());
}
